...
class Parse {
    ...
    static List<State> parseClassic(Def def, String line, String var, Meta meta) {
        ...
        List<String> args = prependEach(argList, "*");
        List<State> stated = list();
        if ("SET".equals(type)) {
            stated.add(new SET(var, args, null, meta));
        } else if ("SET_IMPLIED".equals(type)) {
            stated.add(new SET(var, null, args, meta));
        } else if ("EQL".equals(type)) {
            stated.add(new EQL(var, args.get(0), args.subList(1, args.size()), meta));
        } else if ("NOT".equals(type)) {
            stated.add(new NOT(var, args.get(0), meta));
        } else if ("IMP".equals(type)) {
            stated.add(new IMP(var, args.get(0), args.get(1), meta));
        } else if ("AEA".equals(type)) {
            stated.add(new AEA(var, meta));
        } else if ("EXE".equals(type)) {
            stated.addAll(parseExe(var, args, meta, def.name()));
        } else if ("UNQ".equals(type)) {
            stated.add(new UNQ(var, args.get(0), meta));
        } else if ("AND".equals(type)) {
            stated.add(new AND(var, args, meta));
        } else if ("OR".equals(type)) {
            stated.addAll(parseOr(var, meta, args, def.name()));
        } else if ("NEQ".equals(type)) {
            stated.addAll(parseNeq(var, meta, args, def.name()));
        } else if ("IFF".equals(type)) {
            stated.addAll(parseIff(var, meta, args, def.name()));
        } else if ("NEX".equals(type)) {
            stated.addAll(parseNex(var, meta, args, def.name()));
        } else if ("ALL".equals(type)) {
            stated.addAll(parseAll(var, meta, args));
        } else if ("ELT".equals(type)) {
            stated.add(parseElt(line, var, meta));
        } else if ("LOG".equals(type)) {
            stated.add("*TRUE".equals(var) ? LOG.TRUE : LOG.FALSE);
        } else {
    ...
    static List<State> parseAll(String var, Meta meta, List<String> args) {
        List<State> alls = list();
        List<String> vars = splitList(var, ",");
