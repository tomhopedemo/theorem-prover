...
public class Def {
    ...
    List<State> cloneStates(boolean includeName, boolean isCopy, List<State> states, Map<String, String> oldNew) {
        ...
        for (State state : states) {
            ...
            cloned.add(newState);
            oldNew.put(state.var(), newState.var());
        }
    ...
    Map<String, String> copyStateDepsAndAddToDef(State state) {
        ...
        addStates(clone.states());
        return clone.getOldNewVarMap();
    }
    ...
    abstract static class State {
        final String var;
        Meta meta;
        List<String> args;  //var immutable/final //immutable, can be reset in updateRefs, pullUpGuides //immutable but can be reset in updateRefs, addEqlArgs
        ...
        static class Guide {
            ...
            Guide(List<String> vars, List<Integer> fixedIndices, int level, String fixedAlternative) {
                this.vars = vars;
                this.fixedIndices = new ArrayList<>(fixedIndices);
                this.level = level;
            ...
            Guide(String rawGuide) {
                ...
                for (char character : Util.get(splitSlash, 0).toCharArray()) {
                    if (character == '{') {
                        ...
                        if (varPart.length() > 0) {
                            varsAndStaticParts.add("*" + varPart); //add the var - and kill the var part
