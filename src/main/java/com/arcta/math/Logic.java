...
import java.util.*;
import static com.arcta.math.Logic.LogicianType.*;
...
class Logic {
    ...
    class D_3 extends Reduction {
        boolean reduce_internal_single() {
            ...
            Map<String, Def.State> map = def.getVarToStateMap();
            Map<NOT, NOT> notMap = map(); Map<NOT, NOT> topLevelMap = map();
            ...
            boolean updated = topLevelMap.size() > 0; return updated;
        }
    }
        ...
        boolean reduce_internal_single() {
            ...
            for (IMP imp : def.reachImps()) {
                if (reach.contains(map.get(imp.getTruthA()))) {
                    ...
                    def.updateStates(Map.of(imp.var(), imp.getTruthB())); return true;
                }
            }
            return false;
        }
    ...
    class D_5 extends Reduction {
         boolean reduce_internal_single() {
             ...
             for (EQL eql : filter(eqls, eql -> elts.contains(eql.getEqlArgs().get(0)))) {
                 ...
                 def.updateStates(Map.of(eql.var(), TRUE.var())); return true;
             }
             return false;
         }
    }
        ...
        boolean deduce_internal_single() {
            ...
            for (EQL eql1 : def.eqls()) {
                for (EQL eql2 : filter(def.reachEqls(), e -> !(e.var().equals(eql1.var()) && e.getSet().equals(eql1.getSet())) && intersection(e.getEqlArgs(), eql1.getEqlArgs()) && diff(e.getEqlArgs(), eql1.getEqlArgs()))) {
                    eql1.addEqlArgs(diffSet(eql2.getEqlArgs(), eql1.getEqlArgs())); b = true;
                }
            }
            return b;
        }
    ...
    class E_2 extends Deduction {
        boolean deduce_internal_single() {
            List<UNQ> unqs = list(); Set<String> newUnqEltProvs = set();
    ...
    abstract class Deduction {
        ...
        boolean deduce(){
            ...
            long time = timeEnd();
            if (Control.DEBUG_LEVEL > 0) sout("DEBUG: " + b + " lc: " + deductionsLoopCount + "/" + internalLoopCount + " - " + this.getClass().getSimpleName() +  time + "ms"); return b;}
        boolean deduce_internal() {
            internalLoopCount = 0; boolean b = false;
            while (true){
                boolean b1 = deduce_internal_single();
                internalLoopCount++; b |= b1; if (!b1) break;
            }
            return b;
        }
    ...
    abstract class Reduction {
        ...
        boolean reduce(){
            ...
            long time = timeEnd();
            if (Control.DEBUG_LEVEL > 0) sout("DEBUG: " + b + " lc: " + reductionsLoopCount + "/" + internalLoopCount + " - " + this.getClass().getSimpleName() + " ms: " + time); return b;
        }
        boolean reduce_internal() {
            internalLoopCount = 0; boolean b = false;
            while (true){
